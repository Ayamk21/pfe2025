...
const Conversations = () => {
    ...
    const handleDeleteConversation = async (conversationId) => {
        ...
        toast((t) => (
            <div className="flex items-center space-x-4 p-2 bg-white rounded-lg shadow-lg">
                <p className="text-gray-800">Êtes-vous sûr de vouloir supprimer cette conversation ?</p>
                <div className="flex space-x-2">
                    <button
                        ...
                        onClick={async () => {
                            ...
                            try {
                                await axios.delete(`http://localhost:5000/conversation/${conversationId}`, {
                                ...
                                toast.error("Erreur lors de la suppression de la conversation."); // Affiche un toast d'erreur
                            }
                        }}
                    >
                        Oui
            ...
            style: { // Style pour s'assurer que le toast est bien visible
    ...
    const getAvatarFallback = (user) => {
        if (!user) return '❓';
        return user.nom ? user.nom.charAt(0).toUpperCase() : user.email ? user.email.charAt(0).toUpperCase() : '👤';
    };
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 py-12 px-4 sm:px-6 lg:px-8">
            <div className="max-w-3xl mx-auto bg-white p-8 rounded-xl shadow-2xl border border-blue-200">
                ...
                {loading ? (
                    <div className="text-center text-gray-600 text-lg">Chargement des conversations...</div>
                ) : error ? (
                    <div className="text-center text-red-600 text-lg">{error}</div>
                ) : conversations.length === 0 ? (
                    <div className="text-center p-6 bg-blue-50 rounded-lg border border-blue-200">
                        <p className="text-gray-600 text-lg font-medium mb-2">Aucune conversation pour le moment.</p>
                        <p className="text-gray-500 text-md">Commencez une discussion en contactant un autre utilisateur !</p>
                    </div>
                ) : (
                        ...
                        {conversations.map((conv) => {
                            ...
                            return (
                                ...
                                >
                                    ...
                                    >
                                        <div className="absolute inset-0 bg-gradient-to-r from-blue-50 to-blue-100 opacity-0 group-hover:opacity-100
                                                    transition-opacity duration-300 transform scale-x-0 group-hover:scale-x-100 origin-left"></div>

                                        <div className="relative z-10 w-12 h-12 flex items-center justify-center rounded-full bg-blue-500 text-white
                                                            font-bold text-xl shadow-lg flex-shrink-0 mr-4">
                                            {receiver.profilePicture ? (
                                                <img src={receiver.profilePicture} alt={receiver.nom} className="w-full h-full rounded-full object-cover" />
                                            ) : (
                                        ...
                                        <div className="relative z-10 flex-grow">
                                            ...
                                            <p className="text-sm text-gray-500 group-hover:text-gray-700 italic transition-colors duration-300">
                                                Cliquez pour discuter <span className="font-sans">💬</span>
                                            </p>
                                            {conv.updatedAt && (
                                                <p className="text-xs text-gray-400 mt-1">
                                                    Dernière activité : {new Date(conv.updatedAt).toLocaleString()}
                                                </p>
                                            )}
                                        </div>
