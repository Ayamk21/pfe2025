...
function Navbarr({ user }) {
  ...
  const baseNavigation = [
    ...
    { name: 'A propos', href: 'about', current: false, icon: InformationCircleIcon },
    { name: 'Contact', href: 'contact', current: false, icon: EnvelopeIcon },
    { name: 'Actualités', href: 'actualites', current: false, icon: NewspaperIcon },
  ];

  const privateNavigation = current?.role === "etudiant" || current?.role === "bailleur"
    ? [
        { name: 'Communauté', href: 'communaute', current: false, icon: UsersIcon },
        { name: 'Messages', href: 'conversations', current: false, icon: ChatBubbleLeftRightIcon },
        { name: 'Offres de location', href: 'offres', current: false, icon: HomeModernIcon }
      ]
    : [];
  ...
  if (isAuthenticated && current?.role !== "admin") {
    ...
    navigation.push({ name: 'Aide sociale', href: 'aide-sociale', current: false, icon: LifebuoyIcon });
  }
        ...
        <div className="relative flex h-16 items-center justify-between">
          ...
          <div className="flex flex-1 items-center justify-between sm:items-stretch sm:justify-start">
            ...
            <div className="flex items-center flex-shrink-0">
              <Link to="/">
                <img src="assets/b1.png" alt="Binomy logo" className="h-10 w-auto" style={{ width: '150px', height: '60px' }} />
              </Link>
            </div>
            ...
            <div className="hidden sm:ml-6 sm:block">
              <div className="flex space-x-4 h-16 items-center">
                {navigation.map((item) => (
                  ...
                  >
                    {item.icon && <item.icon className="mr-2 h-5 w-5 text-gray-500" aria-hidden="true" />}
                    {item.name}
                  </Link>
                ))}
                {current?.role === "admin" && (
          ...
          <div className="absolute inset-y-0 right-0 flex items-center pr-2 sm:static sm:inset-auto sm:ml-6 sm:pr-0 space-x-2">
            {!isAuthenticated && (
              <>
                ...
                >
                  <UserIcon className="mr-2 h-5 w-5 text-gray-500" aria-hidden="true" />
                  Se connecter
                </Link>
                {/* Register button */}
                <Link
                ...
                >
                  <UserPlusIcon className="mr-2 h-5 w-5 text-gray-500" aria-hidden="true" />
                  S'inscrire
                </Link>
              </>
            )}
        ...
        <div className="space-y-1 px-2 pt-2 pb-3">
          {navigation.map((item) => (
            <DisclosureButton
              ...
              aria-current={item.current ? 'page' : undefined}
            >
              {item.icon && <item.icon className="mr-2 h-6 w-6 text-gray-500" aria-hidden="true" />}
              {item.name}
            </DisclosureButton>
          ))}
          {current?.role === "admin" && (
          ...
          {!isAuthenticated && (
            <>
              ...
              >
                <UserIcon className="mr-2 h-6 w-6 text-gray-500" aria-hidden="true" />
                Se connecter
              </DisclosureButton>
              <DisclosureButton
                as={Link}
              ...
              >
                <UserPlusIcon className="mr-2 h-6 w-6 text-gray-500" aria-hidden="true" />
                S'inscrire
              </DisclosureButton>
            </>
          )}
